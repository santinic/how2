...
function start () {
  ...
  const logText = `${colors.bgBlue.white.bold(' Enter/Space ')} open link ` +
    ...
    `${colors.bgBlue.white.bold(' p ')} print (for copy-paste) ` +
...
function showAnswer (answer) {
  ...
  answerBox.key(['escape', 'q'], () => {
    ...
    if (answersList) {
      // Focus back on the list of answers
      answersList.focus()
    } else {
      // If we skipped the list of answers because there was only one,
      // we re-focus on the main googleList
      googleList.focus()
    }
    screen.render()
  })

  answerBox.key(['b'], (event) => require('openurl').open(answer.link))

  answerBox.key(['p'], function () {
    screen.destroy()
    printAnswer(answer)
  })
