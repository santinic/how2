...
async function fetchQuestionAnswers(parsedLink) {
  ...
  try {
    let results = await context.questions.answers(questionCriteria, [
...
function onErrorSearch(err) {
  ...
  if (msg.indexOf('have detected unusual traffic') !== -1) {
    log.error(
      'You are doing too many requests to Google. You need to wait a bit before trying ' +
        'again.'
    );
...
function selectAnswer({ text, titles, answers, lang, index, remember }) {
  spinner.stop();
  if (answers.length === 0) {
    log.error('Cannot find any reasonable answer for your query.');
    if (!lang) {
      console.log(
        `To get the best answers, make sure you specify the language with ${colors.blue(
          '-l:'
        )}`
      );
      console.log(
        `example: $ how2 ${colors.blue('-l python')} permutations list`
      );
      console.log(
        `example: $ how2 ${colors.blue('-l ruby')} permutations list`
      );
    }
    process.exit(1);
  }
...
async function onAfterSearch(text, links, titles, lang, index) {
  ...
  const selectedLink = links[index],
    parsedLink = utils.parseStackoverflowQuestionId(selectedLink),
    remember = {
      links: links,
      titles: titles
    };
...
function onAfterInteractiveSearch(links, titles) {
  ...
  ui.showGoogleList(links, titles, async index => {
    ...
    var parsedLink = utils.parseStackoverflowQuestionId(selectedLink);
    try {
      let answers = await fetchQuestionAnswers(parsedLink);
      showAnswers(answers);
    } catch (err) {
