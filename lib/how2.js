...
if(protocol !== 'http' && protocol !== 'https') {
  console.log("warning: protocol set to '" + protocol + "', need to be set to 'http' or 'https'. Setting to default setting: 'https'");
  protocol = 'https'
}
...
function searchGoogle(text, lang, callback) {
    ...
    google(searchQuery, function (err, res){
        if(err) {
            return googleError(err, res.links);
        }

        links = res.links.filter(function(link) {
            return link.title !== '';
        })
        .filter(utils.isValidGoogleLink);

        var strippedTitles = res.links.map(function(link) {
